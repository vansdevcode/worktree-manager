...
func (s *Server) buildConfig(table *routing.Table) map[string]any {
	...
	// Build TLS config: use file-based certs if available, fall back to internal issuer.
	tlsConfig := s.buildTLSConfig(subjects)
...
// buildTLSConfig creates the TLS app configuration. It loads file-based certs
// generated by truststore and falls back to Caddy's internal issuer for any
// domains without a cert on disk.
func (s *Server) buildTLSConfig(subjects []string) map[string]any {
	loadFiles := make([]map[string]any, 0)
	uncoveredSubjects := make([]string, 0)
	seen := make(map[string]bool)

	for _, domain := range subjects {
		certFile, keyFile := certs.FindCert(domain)
		if certFile != "" && keyFile != "" {
			// Avoid loading the same cert file multiple times (wildcard covers multiple domains)
			if !seen[certFile] {
				seen[certFile] = true
			...
			uncoveredSubjects = append(uncoveredSubjects, domain)
		}
	}

	tlsConfig := make(map[string]any)

	// Load file-based certificates
	if len(loadFiles) > 0 {
		tlsConfig["certificates"] = map[string]any{
			"load_files": loadFiles,
		}
	}

	// For any domains without file-based certs, use Caddy's internal issuer as fallback
	if len(uncoveredSubjects) > 0 {
		tlsConfig["automation"] = map[string]any{
			"policies": []map[string]any{
				{
					"subjects": uncoveredSubjects,
