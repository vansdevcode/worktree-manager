...
`gh-wtm` has first-class support for working with GitHub pull requests with a **three-tier fallback system**:
...
# Remove current worktree (when inside a worktree directory)
wtm rm
...
One of the most powerful features of `gh-wtm` is **Go template support**. Templates allow you to automatically set up configuration files for each worktree with dynamic variable replacement.
...
Here's a typical development workflow using `gh-wtm`:
